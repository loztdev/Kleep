/**
 * Shared behavioral contract for every `StructuredStore` implementation —
 * run against both `InMemoryStructuredStore` and `SqliteStructuredStore`
 * so they're guaranteed to agree, per NEXT-10.md Tier 6.1's "all Tier 1.2
 * in-memory tests pass against the SQLite impl" acceptance criterion.
 */

import { MemoryKind, Network } from "../../schema";
import type { StructuredStore } from "../types";
import { makeEntry, makeFact, makeOpinion } from "./fixtures";

export function describeStructuredStoreContract(
  name: string,
  makeStore: () => StructuredStore,
): void {
  describe(name, () => {
    ...
    it("filters by entity_id (matches both an entry and asset references)", () => {
      ...
      expect(out.find((a) => a.id === f.id)).toBeTruthy();
    });
