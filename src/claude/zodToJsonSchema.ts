/**
 * Tier 5.5 — structured-output helper: Zod schema → tool-call definition.
 *
 * Converts the subset of Zod node types Kleep's extraction/summarization
 * prompts actually need (object, string, number, boolean, literal, enum,
 * array, record, union, optional, nullable, default) into the JSON Schema
 * shape the Messages API expects for a tool's `input_schema`. Anything
 * outside that subset throws rather than silently producing a schema
 * Claude can't be validated against.
 */

import type { z } from "zod";

/** The `input_schema` shape a Claude `Tool` definition requires for an object schema. */
...
/** Convert a top-level `z.object()` schema into a tool `input_schema`. Throws if the root isn't an object. */
...
function convertByType(schema: z.ZodTypeAny, d: ReturnType<typeof def>): Record<string, unknown> {
  switch (d.typeName as string) {
    ...
    case "ZodEffects":
      ...
      // way back from Claude (see ClaudeClient.structured).
