...
- **5.5 Claude API client** ✅ — `src/claude/`: `ClaudeClient` wraps `@anthropic-ai/sdk` behind a pluggable `ClaudeTransport`. Retry-with-jitter on 429/529/5xx, per-call cost accounting (`CostTracker`, `src/claude/costTracker.ts`), a Zod→tool-schema structured-output helper (`client.structured()`), basic streaming (`client.streamMessage()`), and a fixture record/replay transport (`src/claude/fixtures.ts`) so `npm test` never needs a real key. Manual smoke test: `npm run claude-smoke` (needs `ANTHROPIC_API_KEY`; not run in CI). **Prompt caching** ✅ (new, by request) — `SendMessageOptions.cache`/`cacheTtl` (`src/claude/client.ts`) turns on Anthropic's automatic/top-level `cache_control: {type: "ephemeral", ttl?}` (`ClaudeRequest.cache_control`, `src/claude/types.ts`), threaded through the provider-agnostic layer via `LlmSendOptions.cache`/`cacheTtl` (`src/llm/types.ts`, forwarded by `ClaudeProvider`). `cacheTtl` is Anthropic's own `"5m"`/`"1h"` choice — nothing in between or beyond is possible, since that's the limit Anthropic itself enforces, not a knob Kleep adds. `CostTracker.record()` was fixed in lockstep to price `cache_creation_input_tokens`/`cache_read_input_tokens` at their real 1.25×/0.1× multipliers instead of $0, since it already captured both fields from `Usage` but silently priced them free.
- **5.6 OpenRouter client + generic provider interface** ✅ (new, by request) — `src/llm/`: `LlmProvider` interface (`sendMessage`/`structured`/`streamMessage`/`totalCostUsd`) implemented by `ClaudeProvider` (adapts `ClaudeClient`) and `OpenRouterClient` (new, fetch-based OpenAI-compatible client — retry+jitter, cost from OpenRouter's native `usage.cost`, Zod-backed function-calling, SSE streaming, fixture record/replay). `buildLlmProvider()` + `secureKeyStore.ts` handle connect-time provider selection. Manual smoke test: `scripts/openrouter-smoke.ts` (needs `OPENROUTER_API_KEY`) — **not run**, this dev sandbox's egress policy blocks `openrouter.ai` outright even with a real key provided; verified instead against a mocked `fetch` (`src/llm/openrouter/__tests__/realTransport.test.ts`). **OpenRouter caching** ✅ (new, by request) — two distinct, independently toggleable features, since OpenRouter itself exposes two unrelated caching mechanisms: (1) **prompt caching** — `OpenRouterClient.buildRequest` attaches Anthropic-style `cache_control` (`OpenRouterCacheControl`, `src/llm/openrouter/types.ts`) to the system message and the newest message when `LlmSendOptions.cache` is set, reproducing Anthropic's own automatic-breakpoint behavior with the explicit block-level mechanism OpenRouter actually supports (OpenRouter has no single-field automatic form); same `5m`/`1h` TTL ceiling as calling Anthropic directly, since OpenRouter passes Claude requests straight through. (2) **response caching** — `LlmSendOptions.responseCacheTtlSeconds` sets `X-OpenRouter-Cache`/`X-OpenRouter-Cache-TTL` headers (`RealOpenRouterTransport`), OpenRouter's own exact-request memoization layer with a genuinely configurable 1–86400s TTL — unlike prompt caching this only helps on a byte-identical repeat call (e.g. an accidental double-regenerate), not normal back-and-forth chat, since every turn's request differs. `ConnectScreen` exposes both: a prompt-caching toggle + 5m/1h picker (any provider), and — OpenRouter only — a response-caching toggle + a real duration slider (`@react-native-community/slider`, new dependency).
...
- **Web scope note:** `expo-sqlite` has no sync API on web (see `openKleepDatabase.native.ts`/`.web.ts`) — same policy as `secureKeyStore.ts`: web falls back to a fresh in-memory pipeline and skips the chat list entirely (single ephemeral chat), since web is this project's debug/testing target, not its distribution target. `openKleepDatabase` was split into `.native.ts`/`.web.ts` (was one file with a `Platform.OS === "web"` runtime check) after discovering the web bundle failed outright — Metro resolves imports statically per bundle target, so an unconditional `import "expo-sqlite"` broke the web build even though that code path returned `null` before ever running on web. The web variant now never imports `expo-sqlite`.
...
- **7.2 Why UI integration** 🟡 — `WhyPanel` is wired into the Memory Browser (7.4/7.5): tapping an entity's "Why do I know this?" or an attribute's info icon opens it via `explain()`/`explainAttribute()`. Still missing: the long-press affordance on chat turns themselves (`onJumpToTurn` in `WhyPanel` is plumbed but no caller passes it yet).
- **7.3 Settings** 🟡 — API key entry + provider picker + **live model picker** ✅ (`ConnectScreen.tsx` + `ModelPickerModal.tsx`, a first pass at connect-time only, not a full settings screen). `src/llm/openrouter/models.ts` fetches OpenRouter's public model catalog; `src/claude/models.ts` fetches Anthropic's (needs the API key — `x-api-key`/`anthropic-version` headers, no SDK method for this exists in the pinned `@anthropic-ai/sdk` version). Both normalize to a shared `ModelInfo` shape and thread an `AbortSignal` with a 10s timeout through `withModelFetchTimeout()` so a hung request doesn't run forever after the modal closes. The modal is a searchable list, still just filling in the same free-text field rather than being the only way to pick a model. Disposition sliders and cost dashboard still 📋.
- **7.4 World Bible browser** ✅ — `src/ui/MemoryBrowserScreen.tsx`: a "World Bible" tab lists every `WorldBibleEntry` (`structured.query({kind: MemoryKind.ENTITY})`) as a card (name/type/aliases/summary preview); tapping one opens a detail modal listing every attribute with a per-attribute "why" button (`explainAttribute()` → `WhyPanel`) plus an entity-level one (`explain()`). No edit affordance yet (read-only browser).
- **7.5 Lore book viewer** ✅ — Same screen's "Lore" tab, backed by a new `VectorStore.list(filter?)` method (both `InMemoryVectorStore`/`SqliteVectorStore`) that returns every snippet without needing a query embedding — the browsing use case `query()` (top-K semantic search) was never meant to serve. Detail modal shows full content/tags/network/POV + a "why" button. No tag editor yet.
- **7.5b Relationship graph visualizer (phase 2)** ✅ — Third "Graph" tab on `MemoryBrowserScreen.tsx`, rendered by `RelationshipGraphView.tsx` (new `react-native-svg` dependency — nothing else in the app draws arbitrary shapes/lines). There's no explicit relationship data anywhere in the schema (`WorldBibleAttribute.value` is always a plain scalar, never a reference to another entity), so `src/ui/relationshipGraph.ts`'s `buildRelationshipGraph()` infers edges from co-occurrence: any FACT/RULE/SUMMARY/REFLECTION/OPINION asset or Lore snippet whose `entity_ids` mentions two or more known World Bible entities together becomes an edge between every pair, labeled with that asset's `content` as the "why" (multiple co-occurring assets between the same pair collapse into one edge with several reasons, not duplicate edges). Layout is a deterministic circular placement (`layoutNodesInCircle()`) — no force-directed/physics library is a dependency, revisit if the World Bible ever gets large enough that a circle stops being readable. Tapping a node highlights its edges and opens a small info card listing what it's connected to and the co-occurring content; isolated entities (no known connections yet) still render as their own node.
- **7.6 Reflection inbox** — Surface new `REFLECTION` assets; accept/dismiss with effect application.
- **7.7 System prompts + prompt library** ✅ — `chatReply.generateReply()` gained an optional `systemPrompt` param that fully replaces Kleep's built-in persona when set (the LLM plumbing — `LlmSendOptions.system` — already flowed through both providers; this was UI/storage work, not an architecture change). `PromptStore` (new `InMemoryPromptStore`/`SqlitePromptStore`, same interface-with-in-memory-fallback pattern as `StructuredStore`/`VectorStore` — saved prompts work for the length of a session on web too) persists user-authored prompts; `ChatSessionStore` gained a `system_prompt` column + `updateSystemPrompt()` so each chat can override the app-wide default (set once at connect time in `ConnectScreen.tsx`, threaded through `ConnectedContext.defaultSystemPrompt` the same way `model` is — new chats inherit it, existing chats keep whatever they were created with). `PromptPickerModal.tsx` (Saved/Library tabs, mirrors `ModelPickerModal.tsx`'s conventions) is opened both from `ConnectScreen` (sets the default) and `ChatScreen`'s header (per-chat override). The "Library" tab fetches the public `awesome-chatgpt-prompts` GitHub CSV live (no auth) — there's no "prompt catalog API" the way there is for LLM models, so `src/prompts/promptLibrary.ts` hand-parses the RFC4180-ish CSV (quoted fields, embedded commas/newlines — the real dataset has both) rather than pulling in a CSV dependency for one file; verified against a live snapshot of the real dataset in `promptLibrary.smoke.test.ts` (skips itself if the fixture isn't present — not committed, it's a large third-party file). `withModelFetchTimeout` moved to `src/util/fetchTimeout.ts` as `withFetchTimeout` once the prompt library needed the identical timeout/cancellation logic (re-exported under the old name so nothing else had to change).
