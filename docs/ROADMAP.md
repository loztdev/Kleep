...
- **5.5 Claude API client** ✅ — `src/claude/`: `ClaudeClient` wraps `@anthropic-ai/sdk` behind a pluggable `ClaudeTransport`. Retry-with-jitter on 429/529/5xx, per-call cost accounting (`CostTracker`, `src/claude/costTracker.ts`), a Zod→tool-schema structured-output helper (`client.structured()`), basic streaming (`client.streamMessage()`), and a fixture record/replay transport (`src/claude/fixtures.ts`) so `npm test` never needs a real key. Manual smoke test: `npm run claude-smoke` (needs `ANTHROPIC_API_KEY`; not run in CI). **Prompt caching** ✅ (new, by request) — `SendMessageOptions.cache` (`src/claude/client.ts`) turns on Anthropic's automatic/top-level `cache_control: {type: "ephemeral"}` (`ClaudeRequest.cache_control`, `src/claude/types.ts`), threaded through the provider-agnostic layer via `LlmSendOptions.cache` (`src/llm/types.ts`, forwarded by `ClaudeProvider`; a no-op for `OpenRouterClient`, which has no caching concept). Only `src/ui/chatReply.ts`'s `generateReply()` sets it — its `messages` array grows every turn and will eventually cross a model's minimum cacheable token count, unlike the single-shot extraction/summarization prompts (individually ~112–324 tokens, well under the 4096-token floor for the default `claude-opus-4-8`), which would never see a cache hit. `CostTracker.record()` was fixed in lockstep to price `cache_creation_input_tokens`/`cache_read_input_tokens` at their real 1.25×/0.1× multipliers instead of $0, since it already captured both fields from `Usage` but silently priced them free.
