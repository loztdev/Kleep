...
- Prompt caching (new, by request) — `SendMessageOptions.cache`/`cacheTtl` opts a call into Anthropic's automatic `cache_control` (`5m`/`1h` — Anthropic's own ceiling, not configurable further), threaded through `LlmSendOptions.cache`/`cacheTtl` for provider-agnostic callers; only `chatReply.ts` sets it, since it's the one call site whose prompt reliably grows past a model's minimum cacheable token count. Also wired into `OpenRouterClient` (`src/llm/openrouter/`), which passes Claude requests through to the same Anthropic mechanism via explicit block-level `cache_control` (system message + newest message), plus a second, unrelated feature — OpenRouter's own exact-request *response* cache (`LlmSendOptions.responseCacheTtlSeconds`, `X-OpenRouter-Cache*` headers), which does have a genuinely configurable 1–86400s TTL since it's OpenRouter's own layer, not a provider pass-through. `ConnectScreen` exposes both with a toggle (+ 5m/1h picker, + a real slider for the response-cache TTL — new `@react-native-community/slider` dependency).
