...
- A pushed commit produces a downloadable, installable APK 🟡 — two real CI runs so far: the first built and uploaded `kleep-debug-apk` successfully, but a **debug** build doesn't embed the JS bundle (expects a live Metro server on the dev machine), so it showed a red "Unable to load script" screen on install. Switched to `assembleRelease` (still needs no signing secrets — Expo's generated `build.gradle` points the release signing config at the auto-generated debug keystore by default), which surfaced a second, real issue: Metro couldn't bundle `@anthropic-ai/sdk`'s `node:fs`/`node:path` imports for a native target. Reproduced locally via `expo export --platform android` (same Metro bundling step CI's `assembleRelease` runs) and fixed with the `metro.config.js` stub above — confirmed the local repro now bundles cleanly (823 modules) and that `expo export --platform web` still works too. **Re-verifying the actual CI run next.**
